#!/usr/bin/env python3
"""Top-k closest/farthest aligned teacher-student pairs for MNISTUncond.

Workflow:
1) Sample K shared Gaussian noises z.
...
def save_rank_grid(
    ...
    idx2_all: torch.Tensor,
    teacher_steps: int,
) -> None:
    t_u8 = tensor_to_uint8_nhwc(teacher_imgs[rank_indices])
    s_u8 = tensor_to_uint8_nhwc(student_imgs[rank_indices])
    tc1_u8 = gather_train_images_uint8_nhwc(train_ds, idx1_all[rank_indices])
    tc2_u8 = gather_train_images_uint8_nhwc(train_ds, idx2_all[rank_indices])
    grid = build_grid_rows([t_u8, s_u8, tc1_u8, tc2_u8])
    grid = add_left_row_labels(
        grid,
        [
            f"Teacher {teacher_steps}-step",
            "Student 1-step",
...
def write_summary(
    ...
    with open(out_path, "a", encoding="utf-8") as f:
        ...
        for rank, i in enumerate(indices.cpu().tolist(), start=1):
            f.write(
                f"{rank}\t{i}\t{distances[i].item():.8f}\t"
                f"{idx1_all[i].item()}\t{d1_all[i].item():.8f}\t"
                f"{idx2_all[i].item()}\t{d2_all[i].item():.8f}\n"
            )
        f.write("\n")


def main() -> None:
    parser = argparse.ArgumentParser(description="Plot top-k closest/farthest teacher-student aligned pairs.")
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="/slurm-storage/shucli/PROJECT_FOLDER/DMD2/experiments/MNISTUncond/Top10s",
    )
    ...
    d1_all, d2_all, idx1_all, idx2_all = top2_train_l2(t_flat, train_flat.float(), args.nn_chunk)
    ...
    write_summary(summary_txt, "closest", closest_idx, pair_dist, idx1_all, idx2_all, d1_all, d2_all)
    write_summary(summary_txt, "farthest", farthest_idx, pair_dist, idx1_all, idx2_all, d1_all, d2_all)
