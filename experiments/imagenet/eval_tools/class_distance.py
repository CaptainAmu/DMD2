#!/usr/bin/env python3
"""Per-class mean images from the teacher EDM (ImageNet-64), pairwise L2 between means, heatmap + stats."""
...
def main():
    ...
    parser.add_argument("--num_sample_classes", type=int, default=100)
    ...
    perm = torch.randperm(args.label_dim, generator=g)[: args.num_sample_classes]
    class_indices = [int(x) for x in perm.tolist()]
    k = len(class_indices)
    ...
    summary_lines = [
        f"Sampled {k} ImageNet class indices (seed={args.seed}):",
        str(class_indices),
        "",
    ...
    ax.set_title("L2 distance between per-class mean images (teacher, sampled classes)")
    ax.set_xlabel("sampled class slot")
    ax.set_ylabel("sampled class slot")
